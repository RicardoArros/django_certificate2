...
def registro(request):
    ...
    #
    try:
        Certificado.objects.create(id_certificado=id_certificado, nombre=nombre,
                                   fecha=fecha, curso=curso, version=version, id_verificacion=id_verificacion)
...
def editar(request):
    ...
    if request.method == 'GET':
        print('Si es GET pasamos por aqui')
        try:
            cert = Certificado.objects.get(
                id_certificado=request.GET["txtValidador"])

            visibilidad = "visible"
            cert.fecha = cert.fecha.strftime('%Y-%m-%d')
            return render(request, "actualizar.html", {"form2": visibilidad, "cert": cert})
        except:
            cert = None
    else:
        print('Si es POST pasamos por acasote')
        if cert == None:
            id_certificado = None
...
def valida(request):
    ...
    try:
        certificado = Certificado.objects.get(id_certificado=request.GET["txtCertificado"])
        if certificado.id_verificacion != request.GET['txtValidador']:
            message = 'Certificado invalido'
    except Exception as ex:
        message = 'Ocurrio un inconveniente validando el certifico (1)'
    except Error as err:
        message = 'Ocurrio un inconveniente validando el certifico (2)'
    return render(request, "validar.html", {'message': message})
